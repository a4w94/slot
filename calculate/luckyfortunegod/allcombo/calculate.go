...
import (
	...
	"math/rand"
	per "permutation/luckyfortunegod"
...
type TotalResult struct {
	...
	NGScatterCombo    [comboresultnum]int
...
var ngcombototal int
var fgcombototal int
...
func Run(session int, runtime int, costlevelinput int, rtp int) {
	...
	*m = costlevelinput / bet
	...
	cn, cf := &ngcombototal, &fgcombototal
	*cn, *cf = per.Eachreellen("NG", rtp), per.Eachreellen("FG", rtp)
	...
	for i := 0; i < runtime; i++ {
		...
		fmt.Println("ScatterCombo")
		fmt.Println(a.NGScatterCombo)
...
func Totalresult(session int) TotalResult {
	...
	for k := 0; k < session; k++ {
		...
		Total.NGScatterCombo = a.NGScatterCombo
...
func eachroundresult() EachRoundResult {
	...
	var ngstri = per.Ngstritable
	var fgstri = per.Fgstritable
	fmt.Println(ngstri)
	fmt.Println(fgstri)
			...
			for c := 0; c < len(ngstri[2]); c++ {
				for d := 0; d < len(ngstri[3]); d++ {
					for e := 0; e < len(ngstri[4]); e++ {
						...
						///判斷進FG,並計分///
						if resultmap > 2 {
							Eachresult.NGScatterCombo[resultmap]++
						...
						for k := 0; k < col; k++ {
							...
							///計分///
							Eachresult.NGpay += paytable[symbocombototal[k][0]][symbocombototal[k][1]] * symbocombototal[k][2] * Betmultiple
						}
	...
	// 					///陣列combo累加///
	// 					for k := 0; k < col; k++ {
	// 						Eachresult.FGresult[symbocombototal[k][0]][symbocombototal[k][1]] += symbocombototal[k][2]
	// 						///計分///
	// 						Eachresult.FGpay += paytable[symbocombototal[k][0]][symbocombototal[k][1]] * symbocombototal[k][2] * Betmultiple
	// 					}
	// 					/////////////
